...
This dataset documents **Angolar / Ngola** only. In this repository it is the principal Angola-related creole. The lexicon is here, not in `dictionary/angola/`.
...
- **License:** CC BY 4.0
- **Use for Angolar:** Angolar column of anatomy and numeral tables; minimal-pair glosses labelled Angolar
- **Do not use:** Santome or Lung’Ie columns from the same tables
- **Disputed item:** Angolar ‘two’ as `[ˈdosʊ]` here versus APiCS/Maurer `rôthu`. Both are kept.
